...
router.get("/", authorization, async (req, res) => {
    try {
        if(req.user.name === 'lead'){
          ...
          const classroomPresentations = await pool.query("SELECT date_trunc('month',connections.connection_date) as mon, COUNT(*) as presentations, connections.school FROM connections LEFT JOIN users ON users.user_id = connections.user_id WHERE connections.contact_method = 'classroom presentation' GROUP BY mon, connections.school")
          const groupSessions = await pool.query("SELECT date_trunc('month',connections.connection_date) as mon, COUNT(*) as groups, connections.school FROM connections LEFT JOIN users ON users.user_id = connections.user_id WHERE connections.contact_method = 'group' GROUP BY mon, connections.school")
          const checkins = await pool.query("SELECT date_trunc('month',connections.connection_date) as mon, COUNT(*) as checks, connections.school FROM connections LEFT JOIN users ON users.user_id = connections.user_id WHERE connections.contact_method = 'check-in' GROUP BY mon, connections.school")
